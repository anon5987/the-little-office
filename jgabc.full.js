...
var trimStaff=function(curStaff,width){
  var x;
  var staffUse=$(curStaff).find("use").filter(function(){ return this.href && this.href.baseVal === '#staff'; });
  if(width){
    x = width;
  } else {
      ...
      if(txt) {
        if(firstText && match[3]) {
          ...
          if(header["initial-style"]!="0") {
            ...
            staffInfo.x=startX;
            var useStaff = $(curStaff).find("use").filter(function(){ return this.href && this.href.baseVal === '#staff'; })[0];
